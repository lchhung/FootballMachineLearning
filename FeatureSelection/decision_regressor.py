...
def main():
    ...
    final_X = pd.DataFrame(StandardScaler().fit_transform(X), columns=X.columns)


    # final_X, y_true = make_regression(n_samples=1000, n_features= 94, n_informative=5, random_state=42)
    ...
    importance_list = pd.DataFrame({"feature": f_list, "score": score})

    print(importance_list)

    sfm = SelectFromModel(model, threshold=0.02)

    X_train, X_test, y_train, y_test = train_test_split(final_X, y_true, test_size=0.4, random_state=42)

    sfm.fit(X_train, y_train)

    for feature_list_index in sfm.get_support(indices=True):
        print(f_list[feature_list_index])
