...
def main():
    ...
    attachking = ['H_ATTACKING', 'A_ATTACKING', 'H_CROSSING', 'A_CROSSING',
                  'H_FINISHING', 'A_FINISHING', 'H_HEADING_ACCURACY', 'A_HEADING_ACCURACY',
                  'H_SHORT_PASSING', 'A_SHORT_PASSING', 'H_VOLLEYS', 'A_VOLLEYS',
                  'H_AttackingWorkRate', 'A_AttackingWorkRate']

    # final_dataset.drop(attachking, axis=1, inplace=True)

    MOVEMENT= ['H_Acceleration', 'A_Acceleration',
               'H_Reactions', 'A_Reactions', 'H_Balance', 'A_Balance']

    MOVEMENT_REMAIN = ['H_SprintSpeed', 'A_SprintSpeed', 'H_Agility', 'A_Agility']

    # final_dataset.drop(MOVEMENT, axis=1, inplace=True)

    POWER= ['H_ShotPower', 'A_ShotPower', 'H_Jumping', 'A_Jumping', 'H_Stamina', 'A_Stamina', 'H_Strength',
            'A_Strength', 'H_LongShots', 'A_LongShots']
    ...
    X_final_test, X_validation, y_final_test, y_validation = train_test_split(X_test, y_test, test_size=0.5, random_state=42)
    print(X_train.shape)

    # Check percent of data divided
    for dataset in [y_train, y_final_test, y_validation]:
        percent = round((len(dataset) / len(y_true)), 2)
        print(percent)


    model_name = "RandForest"
    model = RandomForestClassifier(random_state=42)
    cv = 10 # Divide into 5 cross validation dataset
    ...
    #
    # report = classification_report(y_final_test, y_pred, digits=4, output_dict=True)
    # # current_accuracy = report['accuracy']
    # # current_weighted_f_score = report['weighted avg']['f1-score']
    # current_cohen_kappa_score = cohen_kappa_score(y_final_test, y_pred)
    #
